<#include "/layout/header.ftl">
    ...
    <table class="table table-hover">
      ...
      <#list grammarMetas as gm>
        <tr><td>${gm.id}</td><td><a href="${basePath}/grammar/${gm.id}">${gm.name}</a></td><td>${(gm.description)!"------- No decription -------"}</td><td>${gm.date}</td><td><div class="btn-group pull-right"><a class="btn btn-small" href="${basePath}/edit-grammar/${gm.id}"><i class="icon-file"></i></a><a class="btn btn-small" href="${basePath}/delete-grammar/${gm.id}"><i class="icon-remove"></i></a></div></td></tr>
      </#list>
    </table>
